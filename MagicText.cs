...
public class MagicText : MonoBehaviour
{
    ...
    /// <summary>
    /// Inicializa los niveles de dificultad y sus características
    /// </summary>
    ...
    /// <summary>
    /// Actualiza el estado del juego y maneja la entrada del usuario
    /// </summary>
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.V))
            {
                // Confirmar sin eliminar
                QuitarOutlineSinEliminar();
        }
    ...
    // Nueva función para quitar objetos del layer Outline sin eliminarlos
    void QuitarOutlineSinEliminar()
    {
        ...
        // Variable para contar cuántos objetos procesamos
        int objetosProcesados = 0;

        // Recorrer todos los objetos encontrados y cambiarlos de vuelta al layer Default
        foreach (Collider collider in objectsInLayer)
        {
            // Obtener el GameObject al que pertenece el collider
            GameObject obj = collider.gameObject;

            // Obtener el componente ObjetoInteractivo si existe
            ObjetoInteractivo objInteractivo = obj.GetComponent<ObjetoInteractivo>();

            if (objInteractivo != null)
            {
                // Llamar al método QuitarOutline del objeto
                objInteractivo.QuitarOutline();
            }
            else
            {
                // Si no tiene el componente, simplemente cambiar el layer
            ...
            string nombreObjeto = obj.name;
            Debug.Log("Objeto '" + nombreObjeto + "' volvió al layer Default");
        }

        // Log con información sobre cuántos objetos se procesaron
        if (objetosProcesados > 0)
        {
            ultimoObjetoIluminado = null;
            Debug.Log("Se quitó el Outline de " + objetosProcesados + " objetos");
        }
        else
        {
            Debug.Log("No se encontraron objetos en el layer Outline");
        }
    }
    ...
    void CambiarLayerObjeto()
    {
        ...
        if (coincidenciaExacta != null && nivelActual != 3) // No usar coincidencia exacta en nivel 4
        {
            ...
            {
                coincidenciaExacta.AplicarParpadeoMultiple(contadorPulsaciones, 0.5f);
            }
            else // Otros niveles
            {
    ...
    /// <summary>
    /// Inicia la animación de texto y configura el nivel actual
    /// </summary>
    void StartAnimation()
    {
    ...
    /// <summary>
    /// Selecciona una palabra aleatoria según la categoría actual
    /// </summary>
    ...
    /// <summary>
    /// Procesa la palabra seleccionada según el nivel actual del juego
    /// </summary>
    ...
    /// <summary>
    /// Actualiza el color del texto según la emoción actual en el nivel 7
    /// </summary>
